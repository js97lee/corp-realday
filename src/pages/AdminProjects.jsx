...
function AdminProjects() {
  ...
  const loadProjects = async () => {
    try {
      ...
      console.error('❌ [프로젝트 관리] 프로젝트 목록 불러오기 실패:', err)
      console.error('📍 에러 위치: AdminProjects.jsx > loadProjects()')
      console.error('🔍 에러 상세:', {
        message: err.message,
        status: err.status,
        details: err.details,
      ...
      // 에러 메시지 구성
      let errorMessage = '❌ 프로젝트 관리 - 프로젝트 목록을 불러올 수 없습니다.\n\n'
      errorMessage += '📍 발생 위치: 프로젝트 관리 페이지\n'
      ...
      if (err.status >= 500 && err.status <= 504) {
        const retryCount = parseInt(sessionStorage.getItem('projectsRetryCount') || '0', 10)
        if (retryCount < 3) {
          sessionStorage.setItem('projectsRetryCount', String(retryCount + 1))
          setLoading(false) // 재시도 전에 로딩 상태 해제
          setTimeout(() => {
            console.log(`🔄 [프로젝트 관리] 자동 재시도 중... (${retryCount + 1}/3)`)
            loadProjects()
          }, 3000)
          return // 재시도하는 경우 여기서 종료
        } else {
          // 재시도 횟수 초과 시 재시도 카운터 리셋
          sessionStorage.removeItem('projectsRetryCount')
